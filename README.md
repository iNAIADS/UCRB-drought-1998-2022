# UCRB-drought
This repository stores code for the event-based drought analysis on the Upper Colorado River Basin (UCRB) described in Nagamoto et al, 2025 in revision. 

## Contents

```
├── calculations  #code used in UCRB_Drought_Workflow_Impacts.ipynb
│   ├── calculations.py
│   ├── __init__.py
│   └── setup.py
├── climatic_variables  #code to access variables used in UCRB_Drought_Workflow_Impacts.ipynb
│   ├── catchments_climatic_spatial_aggregation.ipynb
│   ├── Readme.txt
│   └── requirements.txt
├── feature_importance  #code using data calculated in UCRB_Drought_Workflow_Impacts.ipynb
...
│   ├── feature_importance_utils.py
│   └── setup.py
├── plotting  #code used in UCRB_Drought_Workflow_Impacts.ipynb
│   ├── plotting.py
│   ├── __init__.py
│   └── setup.py
├── preprocessing  #code used in UCRB_Drought_Workflow_Preprocessing.ipynb
│   ├── preprocessing.py
│   ├── __init__.py
│   └── setup.py
├── pyeto  #code used in UCRB_Drought_Workflow_Impacts.ipynb from https://github.com/woodcrafty/PyETo/tree/master
│   ├── __init__.py
│   ├── _check.py
│   ├── convert.py
│   ├── fao.py
...
│   └── thornthwaite.py
├── README.md
├── requirements_ucrb-drought_v2.yml
├── UCRB_Drought_Workflow_Preprocessing.ipynb
└── UCRB_Drought_Workflow_Impacts.ipynb
...
When running the  UCRB_Drought_Workflow_Preprocessing.ipynb and UCRB_Drought_Workflow_Impacts.ipynb, intermediate data and outputs will be saved to an OUTPUTS folder located in the directory where this code is used locally.
...
After cloning the repository, `cd` into the repository (`cd path/to/ucrb-drought`) and create an environment using the requirements_ucrb-drought_v2.yml file.

In the same command prompt / terminal opened to the `ucrb-drought` directory, run the following code:

```
mamba create -f requirements_ucrb-drought_v2.yml
```

This will create a conda environment called `ucrb-drought_v2` and download the required packages. Activate this conda environment and create a kernel in Jupyter Lab to run the Notebooks

```
mamba activate ucrb-drought_v2 

python -m ipykernel install --user --name=ucrb-drought_v2
```
...
After opening jupyter lab, open the preprocessing workflow file: `UCRB_Drought_Workflow_Preprocessing.ipynb`. This notebook utilizes python code in `preprocessing` and `pyeto`, the latter of which comes from [PyETo](https://github.com/woodcrafty/PyETo/tree/master). In `UCRB_Drought_Workflow_Preprocessing.ipynb`, streamflow, water temperature, specific conductance, and meteorologic data is preprocessed and cleaned, with these intermediate outputs saved to OUTPUTS. The meteorologic data is collected using Google Earth Engine in `climatic_variables/catchments_climatic_spatial_aggregation.ipynb`, but the raw data is also ready in the INPUTS folder on ESS-DIVE. After running `UCRB_Drought_Workflow_Preprocessing.ipynb` once, you do not need to run this file again, as all data has been saved for analysis.

Then, open the analysis file: `UCRB_Drought_Workflow_Impacts.ipynb`. This notebook utilizes python code in `calculations`, `preprocessing`, and `plotting` for most analysis figure plotting in the paper. Run all cells in the `UCRB_Drought_Workflow_Impacts.ipynb` notebook to complete the analysis. The final cells at the end of each section in the notebook produce visualizations of the data that were used in the associated paper. 

After obtaining the Relative Change percentages using `UCRB_Drought_Workflow_Impacts.ipynb`, the streamflow values along with physical catchment attributes from [GAGESII](https://pubs.usgs.gov/publication/70046617) are used in `feature_importance/permutation_importance/xgb_optimizer_for_regression.ipynb` and `feature_importance/shapley_values/xgb_optimizer_for_regression.ipynb` to investigate with attributes may be predictive of catchment streamflow vulnerability to drought. These notebooks, while separate, use the same virtual environment, and utilize python code in `feature_importance/feature_importance_utils.py`.
...
Nagamoto et al (in revision): Widespread Drought-driven Declines in Streamflows and Water quality in the Upper Colorado River Basin (1998-2022).
...
## License
This code is available with a CCBy4.0 license published in this dataset: [Nagamoto E ; Ciulla F ; Ombadi M ; Willard J ; Carroll R ; Varadharajan C (2025): Dataset: "Widespread Drought-driven Declines in Streamflows and Water quality in the Upper Colorado River Basin (1998-2022)". iNAIADS, ESS-DIVE repository. Dataset. doi:10.15485/2551894 accessed via https://data.ess-dive.lbl.gov/datasets/doi:10.15485/2551894 on 2025-04-25](https://data.ess-dive.lbl.gov/view/doi%3A10.15485%2F2551894). This dataset should be cited for any use of this code. 
